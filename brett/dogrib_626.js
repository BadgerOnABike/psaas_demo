...
(async function () {
    ...
    let projContents = fs.readFileSync(
        "./Dogrib_Dataset/elevation.prj",
        "utf8"
    );
    let elevContents = fs.readFileSync(
        "./Dogrib_Dataset/elevation.asc",
        "utf8"
    );
    ...
    let river = prom.addFileFuelBreak(
        "./Dogrib_Dataset/hydrology_river.kmz"
    );
    river.setName("Rivers");
    let stream = prom.addFileFuelBreak(
        "./Dogrib_Dataset/hydrology_stream.kmz"
    );
    ...
    let polyign = prom.addFileIgnition("2001-10-16T13:00:00",
